...
def eval(parser, dataset):
   ...
   # gt_name_list = sorted(os.listdir(pred_root))
    gt_name_list = sorted([f for f in os.listdir(pred_root) if f.endswith('.png') or f.endswith('.jpg')])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", default='RISNet')  # ✅ 改成你的模型名
    parser.add_argument("--pred_root", default='./results')  # ✅ 去掉子文件夹，主路径
    parser.add_argument("--GT_root", default='../ACOD-12K/Test')  # ✅ 指到 dataset/test
    parser.add_argument("--record_path", default='output/eval_record.txt')  # ✅ 可修改保存路径
    parser.add_argument("--BR", default='on')  # ✅ 开启边界评估
    parser.add_argument("--br_rate", default=15)  # ✅ dilation 大小
    parser.add_argument("--dataset", default='ACOD-12K')  # ✅ 你只评估这个
    args = parser.parse_args()

    # ✅ 直接执行，不需要判断预测目录是否存在
    eval(parser, args.dataset)
